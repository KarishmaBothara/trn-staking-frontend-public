...
const ConfirmationPanel = ({ onFinish }: IProps) => {
  const { setNominationStage, nominate, gasToken, isGasSufficient } = useDeposit();
      ...
      <DrawerContent
        buttonsBar={
          <Box display="flex" flexDirection="row" justifyContent="flex-end" gap={1} width="400px">
            <Button variant="text" onClick={() => setNominationStage(NominationStage.NOMINATION)}>
              Back
            </Button>
            <Button
              variant="contained"
              onClick={handleConfirm}
              disabled={!encodedMessage || !isGasSufficient}
            >
              Continue
            </Button>
          </Box>
