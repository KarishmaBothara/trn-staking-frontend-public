...
import type { CallOptions, CallParam, CallParams } from 'common/types';
...
function subscribe<T>(
  ...
  nextTick((): void => {
    if (mountedRef.current) {
      const canQuery =
        !!fn && (isMapFn(fn) ? fn.meta.type.asMap.hashers.length === validParams.length : true);
  ...
  useEffect((): void => {
    ...
    if (mountedRef.current && fn) {
      ...
      if (
        ...
        subscribe(trnApi, mountedRef, tracker, fn, mappedParams, setValue, options);
      }
    }
  }, [trnApi, fn, options, mountedRef, params]);
